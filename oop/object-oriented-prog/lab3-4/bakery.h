...
/*
 *	Computes a list of the materials from a certain supplier, that are short in supply (also sorted by name), and stores it in the newList
 *	Input: newList, list (pointer to lists), supplier, quantity
 *	Output: -
 */
t_listt	**getSortedListBySupplier(t_listt **newList, t_listt **list, char *supplier, int quantity);
/*
 *	Creates the list for undo
 */
void	append_dem_undo(t_listt **undoList, t_listt **list);
/*
 *	Computes the undo
 *	Input: undoList, List, redoList
 *	Output: 0/1 on failure/success
 */
